...
describe("SpellTokenICO", () => {
    ...
    beforeEach(async function () {
        ...
        contractInstance = await SpellTokenICO.new(tokenInstance.address, daiTokenInstance.address, studentContractAddress, chainLinkETHUSDRinkeby, chainLinkDAIUSDRinkeby, nftSSUTokenAddress);
        await tokenInstance.transfer(contractInstance.address, web3.utils.toBN(1000).mul(bn1e18));
    });
    ...
    describe("buyForETH", function() {
        it("Should buy tokens for ETH successfully with buyForETH", async () => {
            ...
            await contractInstance.buyForETH({ from: payer, value: paymentAmount });
        ...
        it("Should not be able to buy tokens due to no special NFT", async () => {
            ...
            await truffleAssert.reverts(
                contractInstance.sendTransaction( { from: payerWithoutNFT, value: 1000 } ),
                "You need special NFT on you balance to get tokens"
            );
    ...
    describe("buyForDAI", function() {
        it("Should buy tokens for DAI successfully", async () => {
            ...
            await daiTokenInstance.approve(contractInstance.address, web3.utils.toBN(1000).mul(bn1e18), { from: payer });
            ...
            await contractInstance.buyForDAI(daiAmount, { from: payer });
        ...
        it("Should not be able to buy tokens for DAI due to no special NFT", async () => {
            await daiTokenInstance.approve(contractInstance.address, web3.utils.toBN(1000).mul(bn1e18), { from: payerWithoutNFT });
            ...
            const daiAmount = web3.utils.toBN(10).mul(bn1e18);
            await truffleAssert.reverts(
                contractInstance.buyForDAI(daiAmount, { from: payerWithoutNFT }),
                "You need special NFT on you balance to get tokens"
            );
